...
public class Fragment3 extends Fragment {
    ...
    private void initUI(ViewGroup rootView){
        ...
        chart1.setEntryLabelTextSize(10f);
    ...
    private void setData2(HashMap<String, Integer> dataHash2){
        ...
        dataSet2.setColors(Color.rgb(240, 120, 124));
    ...
    private void setData3(ArrayList<Float> dataKeys3, ArrayList<Integer> dataValues3){
        ...
        for(int i = 0; i< dataKeys3.size(); i++){
            try{
                ...
                entries.add(new Entry(outKey, Float.valueOf(outValue)));
            }catch (Exception e){
                e.printStackTrace();
            }
    ...
    public void loadStatData(){
        ...
        String sql = "SELECT MOOD, COUNT(MOOD) FROM "+NoteDatabase.TABLE_NOTE+"" +
                ...
                "GROUP BY MOOD"; //날짜가 한 달 전보다 많고, 내일보다 적을 때
        ...
        // second graph
        sql = "SELECT STRFTIME('%w', CREATE_DATE), AVG(MOOD) " +
        ...
        HashMap<String, Integer> dataHash2 = new HashMap<>(); //요일, mood평균
        for(int i = 0; i<recordCount; i++){
            cursor.moveToNext();
        ...
        // third graph
        sql = "SELECT STRFTIME('%Y-%m-%d', CREATE_DATE), AVG(CAST(MOOD AS REAL)) " +
                "FROM "+NoteDatabase.TABLE_NOTE+" " +
        ...
        for(int i = 0; i<recordCount; i++){
            ...
            int moodCount = cursor.getInt(1); //날짜별 평균 기분
        ...
        cal.add(Calendar.DAY_OF_MONTH, -7);// 오늘에서 7일 빼기

        for(int i = 0; i<7; i++){
            cal.add(Calendar.DAY_OF_MONTH, 1); //하루씩 더한다 6일 전부터 당일까지
            String monthDate = AppConstants.dateFormat5.format(cal.getTime()); //6일 전 데이터부터 시작
            Integer moodCount = recordsHash.get(monthDate); //날짜별 평균 기분을 가져온다

            dataKeys3.add(i-6 * 24.0f); // n시간 전으로 계산?
            if(moodCount == null){
                dataValues3.add(0);
            }else{
                dataValues3.add(moodCount); //해당 날짜의 평균 기분을 추가한다
            }

            AppConstants.println("#"+i+" -> "+monthDate+", "+moodCount);
            // dataKey3는 몇 시간 전인지 (시간으로) 저장
            // dataValues3는 해당 날짜의 평균 기분을 저장
        }
    ...
    public String getDayBefore(int amount){
        ...
        Calendar cal = Calendar.getInstance();
        cal.setTime(todayDate);
        cal.add(Calendar.DAY_OF_MONTH, (-1 * amount));
